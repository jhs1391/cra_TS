...
export function getProducts() {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const response = await axios.get('https://singforhope.org/wp-json/wp/v2/posts');
      const products = response.data.map((post: any) => ({
        id: post.id,
        name: post.title.rendered, // Use the post title as the product name
...
export function getProduct(slug: string) {
  return async (dispatch: Dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await axios.get(`https://singforhope.org/wp-json/wp/v2/posts?slug=${slug}`);
      const post = response.data[0]; // The response is an array, so we take the first item
      const product = {
        id: post.id,
        name: post.title.rendered, // Use the post title as the product name
        // Add other product properties here
